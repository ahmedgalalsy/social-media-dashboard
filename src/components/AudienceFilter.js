...
const AudienceFilter = () => {
  ...
  const [demographic, setDemographic] = React.useState('age');
  ...
  const handleAddFilter = () => {
    ...
    if (demographic !== 'all') {
      newFilters.push(`Demographic: ${demographic}`);
    }
      ...
      <Box sx={{ 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: expanded ? 2 : 0 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box component="span" sx={{ fontWeight: 'medium' }}>Audience Filters</Box>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <ToggleButtonGroup
              value={chartType}
